package com.cmv.petGenesis.model;

public class ExamBody {
	
	private int idExamBody;
	private String condicionCorporal;
	private String dientes;
	private String mucosa;
	private String lengua;
	private String tonsillas;
	private String faringe;
	private String laringe;
	private String oralAdd;
	private String trufa;
	private String flujoNasal;
	private String respiracion;
	private String narizAdd;
	private String conjuntiva;
	private String esclerotica;
	private String cornea;
	private String iris;
	private String cristalino;
	private String parpados;
	private String ojosAdd;
	private String oreja;
	private String condAudExt;
	private String timpano;
	private String oidosAdd;
	private String traquea;
	private String sonidosPulmonares;
	private String ruidosCardiacos;
	private String toraxAdd;
	private String estomago;
	private String intestinos;
	private String higado;
	private String bazo;
	private String riniones;
	private String vejiga;
	private String abdomenAdd;
	private String submaxilares;
	private String preescapulares;
	private String popliteos;
	private String mesentericos;
	private String gangliosAdd;
	private String testiculos;
	private String prepuecio;
	private String prostata;
	private String vulva;
	private String vagina;
	private String utero;
	private String genitalesAdd;
	private String tumoraciones;
	private String heridas;
	private String parasitos;
	private String pelaje;
	private String gAnales;
	private String gMamarias;
	private String deshidratacion;
	private String tegumentoAdd;
	private String claudicacion;
	private String articulaciones;
	private String huesos;
	private String tejidosBlandos;
	private String extremidadesAdd;
	private String actitud;
	private String reflejos;
	private String cerebro;
	private String medulaEspinal;
	private String sncAdd;
	
	public ExamBody() {
	}
	
	public ExamBody(int idExamBody) {
		this.idExamBody = idExamBody;
	}
	
	public int getIdExamBody() {
		return idExamBody;
	}
	public void setIdExamBody(int idExamBody) {
		this.idExamBody = idExamBody;
	}
	public String getDientes() {
		return dientes;
	}
	public void setDientes(String dientes) {
		this.dientes = dientes;
	}
	public String getMucosa() {
		return mucosa;
	}
	public void setMucosa(String mucosa) {
		this.mucosa = mucosa;
	}
	public String getLengua() {
		return lengua;
	}
	public void setLengua(String lengua) {
		this.lengua = lengua;
	}
	public String getTonsillas() {
		return tonsillas;
	}
	public void setTonsillas(String tonsillas) {
		this.tonsillas = tonsillas;
	}
	public String getFaringe() {
		return faringe;
	}
	public void setFaringe(String faringe) {
		this.faringe = faringe;
	}
	public String getLaringe() {
		return laringe;
	}
	public void setLaringe(String laringe) {
		this.laringe = laringe;
	}
	public String getOralAdd() {
		return oralAdd;
	}
	public void setOralAdd(String oralAdd) {
		this.oralAdd = oralAdd;
	}
	public String getTrufa() {
		return trufa;
	}
	public void setTrufa(String trufa) {
		this.trufa = trufa;
	}
	public String getFlujoNasal() {
		return flujoNasal;
	}
	public void setFlujoNasal(String flujoNasal) {
		this.flujoNasal = flujoNasal;
	}
	public String getRespiracion() {
		return respiracion;
	}
	public void setRespiracion(String respiracion) {
		this.respiracion = respiracion;
	}
	public String getNarizAdd() {
		return narizAdd;
	}
	public void setNarizAdd(String narizAdd) {
		this.narizAdd = narizAdd;
	}
	public String getConjuntiva() {
		return conjuntiva;
	}
	public void setConjuntiva(String conjuntiva) {
		this.conjuntiva = conjuntiva;
	}
	public String getEsclerotica() {
		return esclerotica;
	}
	public void setEsclerotica(String esclerotica) {
		this.esclerotica = esclerotica;
	}
	public String getCornea() {
		return cornea;
	}
	public void setCornea(String cornea) {
		this.cornea = cornea;
	}
	public String getIris() {
		return iris;
	}
	public void setIris(String iris) {
		this.iris = iris;
	}
	public String getCristalino() {
		return cristalino;
	}
	public void setCristalino(String cristalino) {
		this.cristalino = cristalino;
	}
	public String getParpados() {
		return parpados;
	}
	public void setParpados(String parpados) {
		this.parpados = parpados;
	}
	public String getOjosAdd() {
		return ojosAdd;
	}
	public void setOjosAdd(String ojosAdd) {
		this.ojosAdd = ojosAdd;
	}
	public String getOreja() {
		return oreja;
	}
	public void setOreja(String oreja) {
		this.oreja = oreja;
	}
	public String getCondAudExt() {
		return condAudExt;
	}
	public void setCondAudExt(String condAudExt) {
		this.condAudExt = condAudExt;
	}
	public String getTimpano() {
		return timpano;
	}
	public void setTimpano(String timpano) {
		this.timpano = timpano;
	}
	public String getOidosAdd() {
		return oidosAdd;
	}
	public void setOidosAdd(String oidosAdd) {
		this.oidosAdd = oidosAdd;
	}
	public String getTraquea() {
		return traquea;
	}
	public void setTraquea(String traquea) {
		this.traquea = traquea;
	}
	public String getSonidosPulmonares() {
		return sonidosPulmonares;
	}
	public void setSonidosPulmonares(String sonidosPulmonares) {
		this.sonidosPulmonares = sonidosPulmonares;
	}
	public String getRuidosCardiacos() {
		return ruidosCardiacos;
	}
	public void setRuidosCardiacos(String ruidosCardiacos) {
		this.ruidosCardiacos = ruidosCardiacos;
	}
	public String getToraxAdd() {
		return toraxAdd;
	}
	public void setToraxAdd(String toraxAdd) {
		this.toraxAdd = toraxAdd;
	}
	public String getEstomago() {
		return estomago;
	}
	public void setEstomago(String estomago) {
		this.estomago = estomago;
	}
	public String getIntestinos() {
		return intestinos;
	}
	public void setIntestinos(String intestinos) {
		this.intestinos = intestinos;
	}
	public String getHigado() {
		return higado;
	}
	public void setHigado(String higado) {
		this.higado = higado;
	}
	public String getBazo() {
		return bazo;
	}
	public void setBazo(String bazo) {
		this.bazo = bazo;
	}
	public String getRiniones() {
		return riniones;
	}
	public void setRiniones(String riniones) {
		this.riniones = riniones;
	}
	public String getVejiga() {
		return vejiga;
	}
	public void setVejiga(String vejiga) {
		this.vejiga = vejiga;
	}
	public String getAbdomenAdd() {
		return abdomenAdd;
	}
	public void setAbdomenAdd(String abdomenAdd) {
		this.abdomenAdd = abdomenAdd;
	}
	public String getSubmaxilares() {
		return submaxilares;
	}
	public void setSubmaxilares(String submaxilares) {
		this.submaxilares = submaxilares;
	}
	public String getPreescapulares() {
		return preescapulares;
	}
	public void setPreescapulares(String preescapulares) {
		this.preescapulares = preescapulares;
	}
	public String getPopliteos() {
		return popliteos;
	}
	public void setPopliteos(String popliteos) {
		this.popliteos = popliteos;
	}
	public String getMesentericos() {
		return mesentericos;
	}
	public void setMesentericos(String mesentericos) {
		this.mesentericos = mesentericos;
	}
	public String getGangliosAdd() {
		return gangliosAdd;
	}
	public void setGangliosAdd(String gangliosAdd) {
		this.gangliosAdd = gangliosAdd;
	}
	public String getTesticulos() {
		return testiculos;
	}
	public void setTesticulos(String testiculos) {
		this.testiculos = testiculos;
	}
	public String getPrepuecio() {
		return prepuecio;
	}
	public void setPrepuecio(String prepuecio) {
		this.prepuecio = prepuecio;
	}
	public String getProstata() {
		return prostata;
	}
	public void setProstata(String prostata) {
		this.prostata = prostata;
	}
	public String getVulva() {
		return vulva;
	}
	public void setVulva(String vulva) {
		this.vulva = vulva;
	}
	public String getVagina() {
		return vagina;
	}
	public void setVagina(String vagina) {
		this.vagina = vagina;
	}
	public String getUtero() {
		return utero;
	}
	public void setUtero(String utero) {
		this.utero = utero;
	}
	public String getGenitalesAdd() {
		return genitalesAdd;
	}
	public void setGenitalesAdd(String genitalesAdd) {
		this.genitalesAdd = genitalesAdd;
	}
	public String getTumoraciones() {
		return tumoraciones;
	}
	public void setTumoraciones(String tumoraciones) {
		this.tumoraciones = tumoraciones;
	}
	public String getHeridas() {
		return heridas;
	}
	public void setHeridas(String heridas) {
		this.heridas = heridas;
	}
	public String getParasitos() {
		return parasitos;
	}
	public void setParasitos(String parasitos) {
		this.parasitos = parasitos;
	}
	public String getPelaje() {
		return pelaje;
	}
	public void setPelaje(String pelaje) {
		this.pelaje = pelaje;
	}
	public String getgAnales() {
		return gAnales;
	}
	public void setgAnales(String gAnales) {
		this.gAnales = gAnales;
	}
	public String getgMamarias() {
		return gMamarias;
	}
	public void setgMamarias(String gMamarias) {
		this.gMamarias = gMamarias;
	}
	public String getDeshidratacion() {
		return deshidratacion;
	}
	public void setDeshidratacion(String deshidratacion) {
		this.deshidratacion = deshidratacion;
	}
	public String getTegumentoAdd() {
		return tegumentoAdd;
	}
	public void setTegumentoAdd(String tegumentoAdd) {
		this.tegumentoAdd = tegumentoAdd;
	}
	public String getClaudicacion() {
		return claudicacion;
	}
	public void setClaudicacion(String claudicacion) {
		this.claudicacion = claudicacion;
	}
	public String getArticulaciones() {
		return articulaciones;
	}
	public void setArticulaciones(String articulaciones) {
		this.articulaciones = articulaciones;
	}
	public String getHuesos() {
		return huesos;
	}
	public void setHuesos(String huesos) {
		this.huesos = huesos;
	}
	public String getTejidosBlandos() {
		return tejidosBlandos;
	}
	public void setTejidosBlandos(String tejidosBlandos) {
		this.tejidosBlandos = tejidosBlandos;
	}
	public String getExtremidadesAdd() {
		return extremidadesAdd;
	}
	public void setExtremidadesAdd(String extremidadesAdd) {
		this.extremidadesAdd = extremidadesAdd;
	}
	public String getActitud() {
		return actitud;
	}
	public void setActitud(String actitud) {
		this.actitud = actitud;
	}
	public String getReflejos() {
		return reflejos;
	}
	public void setReflejos(String reflejos) {
		this.reflejos = reflejos;
	}
	public String getCerebro() {
		return cerebro;
	}
	public void setCerebro(String cerebro) {
		this.cerebro = cerebro;
	}
	public String getMedulaEspinal() {
		return medulaEspinal;
	}
	public void setMedulaEspinal(String medulaEspinal) {
		this.medulaEspinal = medulaEspinal;
	}
	public String getSncAdd() {
		return sncAdd;
	}
	public void setSncAdd(String sncAdd) {
		this.sncAdd = sncAdd;
	}

	public String getCondicionCorporal() {
		return condicionCorporal;
	}

	public void setCondicionCorporal(String condicionCorporal) {
		this.condicionCorporal = condicionCorporal;
	}
}
